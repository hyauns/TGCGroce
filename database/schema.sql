...
-- Enable row-level security and audit logging
-- These settings should be configured at the database level
...
CREATE TABLE payment_audit_logs (
    ...
    INDEX idx_risk_score_timestamp (risk_score DESC, timestamp DESC),
    INDEX idx_admin_actions (admin_user_id, timestamp DESC) WHERE admin_user_id IS NOT NULL
);
...
CREATE TABLE payment_transactions (
    ...
    -- Indexes
    INDEX idx_customer_processed (customer_id, processed_at DESC),
    INDEX idx_order_id (order_id),
    INDEX idx_transaction_id (transaction_id),
    INDEX idx_status_processed (status, processed_at DESC)
);
...
CREATE INDEX idx_payment_methods_customer ON payment_methods(customer_id, created_at DESC);
CREATE INDEX idx_payment_methods_last4 ON payment_methods(last4, brand);
CREATE INDEX idx_billing_addresses_customer ON billing_addresses(customer_id);
CREATE INDEX idx_audit_logs_high_risk ON payment_audit_logs(risk_score DESC, timestamp DESC) WHERE risk_score >= 7;
...
-- Policy: Customers can only access their own payment methods
CREATE POLICY customer_payment_methods_policy ON payment_methods
    FOR ALL TO application_role
    USING (customer_id = current_setting('app.current_customer_id', true));

-- Policy: Customers can only access their own billing addresses
CREATE POLICY customer_billing_addresses_policy ON billing_addresses
    FOR ALL TO application_role
    USING (customer_id = current_setting('app.current_customer_id', true));

-- Policy: Customers can only view their own audit logs
CREATE POLICY customer_audit_logs_policy ON payment_audit_logs
    FOR SELECT TO application_role
    USING (customer_id = current_setting('app.current_customer_id', true));

-- Admin policies (separate role with elevated privileges)
CREATE POLICY admin_full_access_policy ON payment_methods
    FOR ALL TO admin_role
    USING (true);

CREATE POLICY admin_audit_access_policy ON payment_audit_logs
    FOR ALL TO admin_role
    USING (true);
...
CREATE VIEW safe_payment_methods AS
SELECT 
    id,
    customer_id,
    last4,
...
GRANT SELECT, INSERT, UPDATE, DELETE ON payment_methods TO application_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON billing_addresses TO application_role;
GRANT SELECT, INSERT ON payment_audit_logs TO application_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON payment_transactions TO application_role;

GRANT ALL ON payment_methods TO admin_role;
GRANT ALL ON billing_addresses TO admin_role;
GRANT ALL ON payment_audit_logs TO admin_role;
GRANT ALL ON payment_transactions TO admin_role;
GRANT ALL ON security_incidents TO admin_role;
GRANT ALL ON encryption_key_rotations TO admin_role;

-- Grant access to views
GRANT SELECT ON safe_payment_methods TO application_role;
GRANT SELECT ON payment_method_summary TO application_role;
