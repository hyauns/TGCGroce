...
async function main() {
  ...
  try {
    ...
    const cats = await sql`SELECT category FROM products WHERE category ILIKE '%Flesh%Blood%' LIMIT 1`;
    if (!cats || cats.length === 0) {
        console.log("No category matching '%Flesh%Blood%' found!");
        return;
    }
    ...
    `;
    console.log(`Flesh and Blood - Updated to sealed automatically based on keywords.`);

    // 2. Convert remaining null/empty or ANYTHING ELSE to "single" (as per user's request)
    ...
    `;
    console.log(`Flesh and Blood - Updated remaining product types to Cards (single).`);
